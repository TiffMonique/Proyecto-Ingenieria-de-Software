...
function createUsersValidation(data) {
    const schema = yup.object().shape({ //creamos un objeto yup
        nombre: yup.string().min(4).matches(/^^[a-zA-ZáéíóúÁÉÍÓÚñÑ]+(?: [a-zA-ZáéíóúÁÉÍÓÚñÑ]+)*$/).required(),
        apellido: yup.string().min(4).matches(/^^[a-zA-ZáéíóúÁÉÍÓÚñÑ]+(?: [a-zA-ZáéíóúÁÉÍÓÚñÑ]+)*$/).required(),
